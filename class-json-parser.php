            ...
            $this->debugEcho("is_array, search for subloop-array:<br>");
                ...
                foreach($val as $param1 => $valOfParam1) {
                  ...
                  if (is_object($valOfParam1)) {
                    ...
                    $subloopHTML1 .= $subloopHTMLTmp;
                  } else if (is_string($valOfParam1) || is_numeric($valOfParam1)) {  
                    $subloopHTML2 = $this->replacePattern($subloopHTML2, $param1, $valOfParam1);
                    $subloopHTML1 = $subloopHTML2;
                  } else {
                    $this->debugEcho("not an object, string, numeric: no subloop processing implemented for this JSON-structure<br>");
                  }
                }
                $subloopHTML1 = $this->clearUnusedArrayDatafields($subloopHTML1); ## clear empty {n}-datafiels 
                $sli = '{subloop-array:'.$subloopNode.':'.$subLoopNumber.'}'.$subloopStructure.'{/subloop-array}';
