...
class JsonContentParser123 {
    ...
    /* retrieveDataAndBuildAllHtmlItems: get json-data, build html*/
      ...
      if (!is_object($jsonTree) && !is_array($jsonTree)) {
        $this->debugEcho("<hr>problems with JSON-structure: check basenode-param<hr>", "wordpressticket");
        exit;
      }
    ...
    private function replacePatternWithKeyin($datastructure, $pattern, $value, $keyIn) {
       ...
       if (mb_check_encoding($value, 'UTF-8')) {
        $value = htmlentities($value, ENT_QUOTES, "UTF-8", FALSE); # convert to HTML
       }

      if (mb_check_encoding($keyIn, 'UTF-8')) {
      ...
      if (is_numeric($pattern)) {  # preg_replace: trouble with pattern {0}
        ...
        $datastructure = str_replace("{".$pattern.":htmlAndLinefeed2htmlLinefeed}" , $this->value2htmlAndLineFeed2LineFeed($value) , $datastructure);    # proversion
        ...
        $datastructure = preg_replace("/{".$pattern.":htmlAndLinefeed2htmlLinefeed}/i" , $this->value2htmlAndLineFeed2LineFeed($value) , $datastructure);    # proversion
